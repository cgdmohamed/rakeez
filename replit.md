...
The backend is built with Express.js and TypeScript, following a modular controller-service architecture. It uses JWT for authentication and authorization with role-based access control and OTP verification via Twilio. Password management includes secure change functionality with strong validation, rate limiting, and audit logging. Authentication features a global 401 error handler, unified login, password reset flow, and session protection. An API FAQ page (`/api-faq`) provides comprehensive documentation on integration issues, CORS solutions, authentication, rate limiting, production checklists, and WebSocket setup. All error responses use structured bilingual messages. The project utilizes PostgreSQL via Neon Database with Drizzle ORM, employing JSONB for bilingual content, enum types, soft deletes, and timestamp tracking.
